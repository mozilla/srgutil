...
def test_fetch_json(test_ctx):
    ...
    jdata = test_ctx.impl(IS3Data).fetch_json("http://127.0.0.1:9001/some-nonexistant-url-foo.json")
    assert jdata is None


def test_get_s3_json_content(test_ctx):
    """ Just test an S3 bucket and key that doesn't exist """
    jdata = test_ctx.impl(IS3Data).get_s3_json_content("taar_not_my_bucket", "this/is/not/a/valid/path")
    assert jdata is None
